...
"""Логика сохраниея состояния. работает через указание индекса массива по которому мы итерируемся. 
Текущий индекс щаписывается в файл"""



def wright_pickle(fine_name, data):
    """записать данные в бинароном виде в файл с раширением <.pickle>"""
    with open(f'{fine_name}.pickle', 'wb') as f:
        pickle.dump(data, f)

def read_pickle(fine_name):
    """Прочесть данные из файла с раширением <.pickle>. Если файла нет -вернуть 0"""
    try:
        with open(f'{fine_name}.pickle', 'rb') as f:
            data = pickle.load(f)
        return data
    except:
        return 0
...
def main():
    ...
    start_i = read_pickle('index_i') # прочесть индекс из файла. в случае отсутсвия файла - 0
    """используем генератор натуральных чисел <range>. эта функция генерирует последовательность чисел ОТ заданного К заданному
    Сгенерированные числа используются в качестве индексов к массиу с данными"""
    for i in range(start_i, len(categories_list)):
        """в процессе итерации записываем текущий индекс в файл что бы в случае рестарта начать итерацию с этого индекса"""

        # categories_list[i] - обращаемся к списку словарей по индексу i, получаем словарь. обращаемся к словарю по ключу category_name
        print(f"Текущая категория: {categories_list[i]['category_name']}")
        # получаем номер последней страницы для текущей категории
        last_number_page: int or None = get_last_number_page_category(url=categories_list[i]['category_url'],
                                                                      session=session)
        ...
        product_urls = pagination(url=categories_list[i]['category_url'], session=session, last_number_page=last_number_page)

        category_obj = {
            'category_name': categories_list[i]['category_name'],  # имя категории
            'products': []
        }
        # category_obj > это словарь с двумя ключами. category_name содержит имя категории, а product_urls это

        start_j = read_pickle('index_j') # прочесть индекс из файла. в случае отсутсвия файла - 0
        """по аналогии с вышестоящим циклом..."""
        for j in range(start_j, len(product_urls)):

            while True:
                try:
                    product_info = get_product_details(product_urls[j], session)
                    break
            ...
            wright_pickle('index_j', j)
            with open('products.json', 'w', encoding="utf-8") as f:
                json.dump(result, f, ensure_ascii=False, indent=4)


        wright_pickle('index_i', i)
